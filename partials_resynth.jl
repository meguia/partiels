...
function extendpar(h, N, decimate; retro=0.9, slope = 10.0)
    ...
    lb = [0.0, 0.3]
    ub = [10.0, 2.0]
    fit1 = curve_fit(model1, trs, hrs, p1, lower=lb, upper=ub)
    s2 = padarray(hrs,hrs[end],length(tex))
    cross = sigmoid(tex,retro*trs[end],slope)
    hex =  cross .* model1(tex,coef(fit1)) + (1.0 .- cross).*s2
    return tex,hex
end    
