...
function extendpar(h, Na, decimate; slope = 10.0, thr = -4)
	...
	nmax = argmax(hrs)
	cr1 = findfirst(20*log10.(hrs[nmax:end]) .< thr) 
	if isnothing(cr1)
		cr2 = length(hrs)
	else
		cr2 = cr1 + nmax
	...
	@. model1(x, p) = p[1]*exp(-x*p[2])
    p1 = [0.2, 0.8]
    lb = [0.0, 0.3]
    ub = [10.0, 10.0]
    fit1 = curve_fit(model1, trs[nmax:cr2], hrs[nmax:cr2], p1, lower=lb, upper=ub)
    cross = sigmoid(trs,trs[cr2],slope)
    amp = cross .* model1(trs,coef(fit1)) + (1.0 .- cross).*hrs
    return trs,amp
end    
...
prom $(@bind prom Slider(1.0:0.1:29.0,default=10.0;show_value=true)) \
